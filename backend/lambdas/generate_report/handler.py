    ...
    try:
        class PDF(FPDF):
            ...
            def body(self, text):
                ...
                safe_text = safe(text).strip() or "N/A"
                self.multi_cell(190,6,safe_text); self.ln(4)
        ...
        pdf.multi_cell(190,12,"QUARTERLY IMPACT REPORT",align="C")
        pdf.set_font("Helvetica","B",16); pdf.set_text_color(255,140,0)
        pdf.multi_cell(190,10,safe(str(period) or "Quarterly Report"),align="C")
        pdf.set_font("Helvetica","",12); pdf.set_text_color(80,80,80)
        pdf.multi_cell(190,8,safe(f"Organization: {str(ngo_name) or 'NGO'}"),align="C")
        pdf.multi_cell(190,8,f"Date: {datetime.now(timezone.utc).strftime('%B %d, %Y')}",align="C")
        pdf.ln(8)
        ...
        if highlights:
            ...
            for h in highlights:
                ...
                safe_h = safe(str(h).strip() or "Highlight")
                pdf.multi_cell(190,6,f"  * {safe_h}"); pdf.ln(1)
            pdf.ln(3)
